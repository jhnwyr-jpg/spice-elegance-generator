      ...
      <div className="container-premium">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {reviews.map((review, index) => (
            <div 
              key={index}
              className={`group p-6 rounded-2xl bg-card border border-border hover:border-gold hover:shadow-elevated transition-all duration-500 hover-lift cursor-default relative overflow-hidden ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}
            ...
            >
              ...
              <div className="relative z-10">
                ...
                {/* Quote icon */}
                <div className="absolute top-4 right-4 text-4xl text-gold/20 group-hover:text-gold/40 transition-colors">
                  "
                </div>
                
                {/* Review Text */}
                <p className="text-muted-foreground mb-4 leading-relaxed group-hover:text-foreground/90 transition-colors">
                  "{review.text}"
                </p>
                
                {/* Reviewer Info */}
                <div className="flex items-center justify-between pt-4 border-t border-border group-hover:border-gold/30 transition-colors">
                  <div>
                    <p className="font-semibold text-foreground group-hover:text-primary transition-colors">{review.name}</p>
                    <p className="text-sm text-muted-foreground">{review.location}</p>
                  </div>
                  {review.verified && (
                    <div className="flex items-center gap-1 text-xs text-primary group-hover:scale-105 transition-transform">
